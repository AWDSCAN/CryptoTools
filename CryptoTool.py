...
class MainWindows(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.author_text = "作者邮箱：dinqianwxrn@1163.com\n"
        self.load_config()
        self.setWindowTitle('CryptoTools V ' + version + ' ' )
    ...
    def Update(self):
        #打开项目网址github、gitee
        #当用户点击更新时就打开
        webbrowser.open("https://github.com/AWDSCAN/CryptoTools")
...
if __name__ == "__main__":
    ...
    window.show()
    try:
        response = requests.get("version.txt", timeout=2)#检测文件内版本号是否更新
        if (int(response.text.replace('.', '')) > int(version.replace('.', ''))):
            reply = QMessageBox.question(window, '软件更新', "检测到软件已发布新版本，是否前去下载?", QMessageBox.Yes | QMessageBox.No,
                                         QMessageBox.Yes)
            if reply == QMessageBox.Yes:
                webbrowser.open('https://github.com/AWDSCAN/CryptoTools/releases')#打开github、gitee的releases页面下载更新
            else:
                pass
    except:
        pass
